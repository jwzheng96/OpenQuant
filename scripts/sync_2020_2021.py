"""Extend price history backward: 2020-01-01 → 2021-12-31 for all 574 stocks.

Combined with existing 2022-2026 data, gives ~6.4 years of history including
the 2022 bear market + 2021 high-vol period for proper stress test.
"""
    ...
    for i, ts_code in enumerate(ts_codes, 1):
        ...
        if not df.is_empty():
            ...
            if i % 25 == 0 or i == len(ts_codes):
                print(f"  [{i:3d}/{len(ts_codes)}] last={ts_code}", flush=True)
        time.sleep(0.4)
        if i % 30 == 0:
            time.sleep(8)
...
def fetch_basic_one(code: str, indicator: str) -> pl.DataFrame:
    """Use 近五年 period to cover 2020-2021."""
...
def sync_daily_basic(api, ts_codes: list[str]) -> None:
    ...
    t0 = time.time()
    print(f"[basic] {len(ts_codes) * 3} calls (period=近五年)", flush=True)
    for i, ts_code in enumerate(ts_codes, 1):
        code = ts_code.split(".")[0]
...
def main():
    ...
    symbols = api.query.con.execute(
        "SELECT DISTINCT symbol FROM daily WHERE symbol != '000300.SH' ORDER BY symbol"
    ).pl()["symbol"].to_list()
    print(f"extending {len(symbols)} symbols back to {START}", flush=True)

    success = sync_daily(api, ak, symbols)
    sync_daily_basic(api, success)

    # Extend benchmark backward too
    try:
        ...
        if bench is not None and len(bench) > 0:
            ...
            bench = bench.with_columns(
                ...
                (pl.col("trade_date") >= "20200101") & (pl.col("trade_date") <= "20211231")
            ).with_columns(
            ...
            print(f"[bench] wrote {n} rows of 000300.SH (2020-2021)", flush=True)
