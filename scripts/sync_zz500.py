"""M2 universe expansion: add 中证 500 (000905) constituents not already in store.

Combined with existing HS300 head (150 stocks), gives ~600 stock universe.
Pulls daily prices (qfq) + daily_basic in one pass via EM/Sina fallback.
"""
...
def get_zz500_list() -> list[str]:
    ...
    for attempt in range(3):
        try:
            ...
            print(f"  ZZ500 attempt {attempt+1} failed: {e}", flush=True)
            time.sleep(5)
    return []


def sync_daily(api, ak, ts_codes: list[str]) -> list[str]:
    print(f"[daily] {len(ts_codes)} new symbols", flush=True)
    success: list[str] = []
    ...
    for i, ts_code in enumerate(ts_codes, 1):
        ...
        if not df.is_empty():
            ...
            if i % 10 == 0 or i == len(ts_codes):
                print(f"  [{i:3d}/{len(ts_codes)}] ✅ +{ts_code} ({source_used})", flush=True)
        else:
            print(f"  [{i:3d}/{len(ts_codes)}] ⛔ {ts_code}", flush=True)
        time.sleep(0.4)
        if i % 30 == 0:
            print(f"  [pause 8s after {i} requests]", flush=True)
            time.sleep(8)

    if frames:
        big = pl.concat(frames)
        n = api.store.write_daily(big, dataset="daily")
        print(f"[daily] wrote {n} rows ({len(success)}/{len(ts_codes)}) in {time.time()-t0:.1f}s", flush=True)
    return success
...
def sync_daily_basic(api, ts_codes: list[str]) -> None:
    ...
    print(f"[basic] {total} pairs", flush=True)
    for i, ts_code in enumerate(ts_codes, 1):
        code = ts_code.split(".")[0]
        for cn, our in indicators.items():
            ...
            time.sleep(0.3)
        if i % 25 == 0:
            print(f"  [basic {i}/{len(ts_codes)}] pause 5s", flush=True)
            time.sleep(5)
...
def main():
    ...
    zz500 = get_zz500_list()
    new_symbols = sorted(set(zz500) - existing)
    print(f"ZZ500 has {len(zz500)} constituents; adding {len(new_symbols)} new (not in existing)", flush=True)

    if not new_symbols:
        print("nothing to add — universe already covers ZZ500", flush=True)
        return

    success = sync_daily(api, ak, new_symbols)
    if not success:
        print("daily sync produced no new data", flush=True)
        sys.exit(1)
    ...
    print(f"stock_basic: {len(all_symbols)} total", flush=True)
